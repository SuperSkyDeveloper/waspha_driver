...
import {getGDPRComplianceStatementrRequest} from '../../actions/GeneralActions';
...
class GDPRComplianceStatementController extends React.Component {
  ...
  initial = () => {
    const {getGDPRComplianceStatementrRequest} = this.props;

    this.setState({loading: true});
    getGDPRComplianceStatementrRequest((response) => {
      if (response.status) {
      }
...
const actions = {getGDPRComplianceStatementrRequest};
