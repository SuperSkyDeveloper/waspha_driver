...
const TRACKING_BASE_URL = 'https://tracking.waspha.com';
// const TRACKING_BASE_URL = 'https://waspha-tracking-dev.herokuapp.com';
...
class SocketIO {
  ...
  connect(connectCallBack, connectionErrorCallBack = undefined): void {
    ...
    if (!isConnectedWithSocket) {
      console.log('not isConnectedWithSocket');
      // Global events are bound against socket

      this.socket.on('connect_error', (err) => {
        console.log('not isConnectedWithSocket ERR');
      ...
      this.socket.on('connect', () => {
        console.log(' isConnectedWithSocket conenct');

        isConnectedWithSocket = true;

        /* this.socket.off('ping');
        this.socket.on('ping', () => {
          console.log('Socket PING');t
        });
  ...
  getExpressDriverInfo(callback): void {
    this.socket.off('expressDriverInfo');
    this.socket.on('expressDriverInfo', (data) => {
      console.log('expressDriverInfo', data);

      if (callback) {
        if (_.isNil(data.data)) {
          callback(data.isOnline);
        } else {
          callback(data.data.isOnline);
        }
      }
    });
  }
